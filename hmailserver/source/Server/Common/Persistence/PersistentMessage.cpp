   ...
   //---------------------------------------------------------------------------()
   {
      ...
      if (handleFile == INVALID_HANDLE_VALUE || handleFile < 0)
      {
         if (reportError)
         {
            ...
            ErrorManager::Instance()->ReportError(ErrorManager::Medium, 4403, "Message::GetHeader", "Could not read the message header, since the file was not available. File: " + fileName, error_code);
         }
